...
var getEswHelper = {
	/**
	 * These get functions are used to get the value from corresponding custom preferences
	*/	
	...
	getBaseCurrency : function () {
		if(session.getCurrency().currencyCode !== Site.getCustomPreferenceValue("eswBaseCurrency").value){
			return {'value': session.getCurrency().currencyCode};
		}
	...
	*/
	selectCountry : function (country, currency, locale) {
		let eswCurrency = this.createCookie('esw.currency' ,'', '/'),
        ...
        if (empty(allowedMatchCountry)) {
            ...
            if(!empty(countryJson)){
            	for (var key in countryJson.name) {
                    langObj.push({value:key, displayValue: countryJson.name[key]});
                }
            }else {
            	langObj.push({value:dw.system.Site.getCurrent().getDefaultLocale(), displayValue: dw.system.Site.getCurrent().getDefaultLocale()});
            }
    ...
    getCurrenciesOptions: function () {
        ...
        if (empty(allowedMatchCountry)) {
            let countryJson = this.getAllCountryFromCountryJson(country);
            if(!empty(countryJson)){
            	currencyObj.push({value:countryJson.currencyCode, displayValue: countryJson.currencyCode});
            }else {
            	currencyObj.push({value:dw.system.Site.getCurrent().getDefaultCurrency(), displayValue: dw.system.Site.getCurrent().getDefaultCurrency()});
            }
            return currencyObj;
        }
		...
		try {
			...
			if (!empty(roundingModel)) {
				if (roundingModel.model.equalsIgnoreCase('none')) {
					...
					let priceWithoutSecondDecimal = parseInt(price * 10) / 10,
					...
					if (roundingModel.direction.equalsIgnoreCase('up')) {
						...
						let roundedUp = roundingModel.model >= parseInt((price - parseInt(price)) * 100) ? parseInt(price) + roundingModel.model / 100 : parseInt(price) + 1 + roundingModel.model / 100,
	...
	getSubtotalObject: function (cart,isCart, listPrice) {
		var total= 0;
		if(isCart) {
			for each(var productLineItem in cart.productLineItems) {
				total += this.getSubtotalObject(productLineItem, false);
			}
			...
			*/
			...
			for each(var adjustment in cart.priceAdjustments) {
				if (adjustment.appliedDiscount.type == dw.campaign.Discount.TYPE_AMOUNT) {
					...
					if (adjustment.quantity < cart.quantity.value) {
						total += (cart.quantity.value - adjustment.quantity) *  this.getMoneyObject(cart.basePrice.value, false, false).value;
					}
				} else {
					total -= this.getMoneyObject(adjustment.price, false, false).value * -1;
				}
			}
	...
	getOrderDiscount : function(cart) {
		...
		for each(var adjustment in cart.priceAdjustments) {
			if (adjustment.appliedDiscount.type == dw.campaign.Discount.TYPE_AMOUNT) {
				...
				totalDiscount += this.getMoneyObject(adjustment.price, false, false, true).value;
			}
		}
		if (totalDiscount < 0) {
			totalDiscount *= -1;
    ...
    getAllCountryFromCountryJson: function(selectedCountry){
        let siteCountries =  require('*/cartridge/countries'),
	...
	checkRedirect: function() {
		let localeCountry = this.getLanguageFromCountryJson(this.getAvailableCountry());
		let locale  = (localeCountry != null)? localeCountry.locales[0] : request.httpCookies['esw.LanguageIsoCode'].value;
