...
function itemExists(list, pid, config) {
    ...
    if (found && found.productOptionModel && config.options) {
        ...
        if (apiProduct && list && config.qty) {
            try {
                Transaction.wrap(function () {
                    ...
                    if (apiProduct.optionProduct && config.options) {
                        optionModel = apiProduct.getOptionModel();
                        for (var i = 0; i < config.options.length; i++) {
                            var temp = config.options[i];
                            if (temp.optionId && temp.optionMessage && temp.optionVal) {
                                var option = optionModel.getOption(temp.optionId);
                                var optionValue = optionModel.getOptionValue(option, temp.optionVal);
                                optionModel.setSelectedOptionValue(option, optionValue);
                                productlistItem.setProductOptionModel(optionModel);
                                if (temp.optionId === 'Engraved') {
                                    productlistItem.custom.engravedText = temp.optionMessage;
                                } else if (temp.optionId === 'Embossed') {
                                    productlistItem.custom.embossedText = temp.optionMessage;
                                } else if (temp.optionId === 'GiftWrapped') {
                                    productlistItem.custom.giftWrappededText = temp.optionMessage;
                                }
                            }
                        }
                    }
