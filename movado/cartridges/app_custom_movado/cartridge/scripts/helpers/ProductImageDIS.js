...
var cvDISConfiguration =JSON.parse(Site.current.getCustomPreferenceValue('disImageMappingJSON'));
...
function ProductImage(imageObject, viewType, index) {
    ...
    if (!this.image || (cvDISConfiguration.enforceImageScaling && !empty(this.scaleableImage))) {
    ...
    if(!ProductImage.checkIfImageExists(this.image)){
        this.image = null;
    }
}
...
ProductImage.getMediaFile = function(image) {
    var mediaFile = image;
        var baseImageURL = mediaFile.httpURL.toString();
        baseImageURL = baseImageURL.substr( baseImageURL.indexOf("/on/demandware.static/") + 22);
        var baseImageURLArray = baseImageURL.split( '/' );
        var filePathArray  = [ dw.io.File.CATALOGS ];
        for ( var i = 0; i < baseImageURLArray.length; i++) {
            if ( i == 0 || i == 3 ) {
                continue;
            } else if ( i == 1 ) {
        ...
        var filePath = filePathArray.join( dw.io.File.SEPARATOR );
...
ProductImage.checkIfImageExists = function(image){
    // Check existence of the referenced image file.
    var check = true;
	if( !cvDISConfiguration.useMissingImages ) {
		if ( !empty(image) ) {
			if ( !ProductImage.getMediaFile(image).exists() ) {
				check = false;
			}
		} else {
			check = false;
    ...
    if (this.imageObject !== null) {
        ...
        if (images) {
            for (let i = 0; i < images.length; i++) {
                if (i === this.index) {
                    if(ProductImage.checkIfImageExists(images[i])){
                        result.add(this);
                    }
                } else {
                    if(ProductImage.checkIfImageExists(images[i])){
