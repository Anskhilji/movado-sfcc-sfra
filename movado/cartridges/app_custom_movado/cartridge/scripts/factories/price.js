...
function getPrice(inputProduct, currency, useSimplePrice, promotions, currentOptionModel) {
    ...
    priceBook = getProductPriceBook(priceModel);
    salesPrice = priceModel.price;

    if ((priceBook && priceBook.ID.toLowerCase().indexOf(Constants.ECOM_SALE_PRICE_BOOK) == -1) && promotionPrice && promotionPrice.available && salesPrice.compareTo(promotionPrice)) {
 ...
 * @return {dw.value.Money} - List price
 */
 function getProductPriceBook(priceModel) {
    var priceBook;
