...
/**
 * This function is used to check the order status which is passed form API end point
...
function getOrderCountByState(orderState) {
    ...
    var logLocation = 'SwellOrderModel~orderCountByState';
    var orderCount;
    var orderItrator;
    try {
...
function getOrderCountByVolume(numberOfDays) {
    ...
    var logLocation = 'SwellOrderModel~orderCountByVolume';
...
function prepareOrderLineItemsJSON(order) {
    ...
    while (prodLineItemIt.hasNext()) {
        ...
        swellRedemptionID = productLineItem.custom.swellRedemptionId ? productLineItem.custom.swellRedemptionId : null;
        swellPointsUsed = productLineItem.custom.swellPointsUsed ? productLineItem.custom.swellPointsUsed : null;
        ...
        var productPriceAjustments = new Array();
        var productPriceAjustmentsIt = productLineItem.getPriceAdjustments().iterator();

        while (productPriceAjustmentsIt.hasNext()) {
            var productPriceAjustment = productPriceAjustmentsIt.next();
            var priceAjustment = {
            ...
            productPriceAjustments.push(priceAjustment);
        }
        ...
        item = {
            ...
            vendor: null,
            price_adjustments: productPriceAjustments
        };
...
function prepareOrderJSON(order) {
    ...
    var YotpoLogger = require('~/cartridge/scripts/yotpo/utils/YotpoLogger');
    var YotpoUtils = require('~/cartridge/scripts/yotpo/utils/YotpoUtils');
    var logLocation = 'SwellOrderModel~prepareOrderJSON';
    ...
    try {
        ...
        throw Constants.EXPORT_SWELL_ORDER_ERROR;
    }
...
function prepareOrdersJSON(orderIterator) {
    var YotpoLogger = require('~/cartridge/scripts/yotpo/utils/YotpoLogger');
    var logLocation = 'SwellOrderModel~prepareOrdersJSON';
